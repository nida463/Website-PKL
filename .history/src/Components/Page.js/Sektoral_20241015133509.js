...
const Sektoral = () => {
    ...
    try {
      const response = await axios.get("http://116.206.212.234:4000/data-sektoral", {
        params: {
          ...
          page: currentPage, // Gunakan currentPage yang terbaru
          per_page: itemsPerPage, // Kirim jumlah item per halaman
        },
      });
  ...
  // Panggil fetchResults setiap kali currentPage berubah
  useEffect(() => {
    if (selectedOPD || selectedUrusan || DariTahun || SampaiTahun) {
      fetchResults();
    }
  }, [currentPage]); // Hanya dijalankan ketika currentPage berubah

  // Menghandle pencarian data (reset ke halaman 1)
  const handleSearch = async (e) => {
    e.preventDefault();
    setCurrentPage(1); // Reset ke halaman pertama saat melakukan pencarian baru
    await fetchResults(); // Lakukan pencarian baru
  };
