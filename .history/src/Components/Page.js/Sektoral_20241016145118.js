...
const Sektoral = () => {
  ...
  const [totalPages, setTotalPages] = useState(0); // Tambahkan state untuk totalPages
  ...
  const handleSearch = async (e = null) => {
    ...
    try {
      ...
      // Menghilangkan duplikasi berdasarkan id
      const uniqueData = [];
      const seenIds = new Set();

      fetchedData.forEach(item => {
        if (!seenIds.has(item.id)) { // Ganti 'id' dengan atribut unik yang sesuai
          seenIds.add(item.id);
          uniqueData.push(item);
        }
      });

      // Update dataHasil dengan data baru dan hapus data sebelumnya
      ...
      if (totalItems) {
        ...
        setTotalPages(Math.ceil(totalItems / itemsPerPage)); // Hitung total halaman
      } else {
        setTotalData(0);
        setTotalPages(0); // Reset totalPages jika tidak ada totalItems
      }
