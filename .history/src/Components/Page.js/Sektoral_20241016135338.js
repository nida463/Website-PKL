...
const Sektoral = () => {
  ...
  const handleSearch = async (e = null) => {
    ...
    try {
      ...
      // Menggunakan Set untuk menghilangkan duplikasi berdasarkan ID atau properti unik lainnya
      const uniqueData = Array.from(new Set(response.data.map(item => item.id_dssd)))
        .map(id => response.data.find(item => item.id_dssd === id));
