...
const Sektoral = () => {
  ...
  const handleSearch = async (e = null) => {
    ...
    try {
      ...
      // Simpan data hasil pencarian, filter untuk menghindari duplikasi
      const fetchedData = response.data;

      // Gunakan Set untuk menyimpan data unik berdasarkan ID atau atribut lain yang relevan
      const uniqueData = Array.from(new Set(fetchedData.map(item => item.id))) // ganti 'id' dengan atribut unik yang relevan
        .map(id => fetchedData.find(item => item.id === id)); // ganti 'id' dengan atribut unik yang relevan

      setDataHasil(uniqueData);

      // Ambil total data dari header respons
  ...
  const handlePageChange = (pageNumber) => {
    if (pageNumber < 1 || pageNumber > totalPages) return; // Prevent invalid page
    setCurrentPage(pageNumber);
    handleSearch(); // memanggil handleSearch untuk mengupdate data di halaman yang sesuai
  };
