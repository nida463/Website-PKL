...
const handleSearch = async (e = null) => {
  ...
  try {
    ...
    setDataHasil((prevData) => {
      const existingIds = new Set(prevData.map(item => item.id)); // Ganti `id` dengan properti unik yang ada pada data
      const filteredNewData = newData.filter(item => !existingIds.has(item.id)); // Ganti `id` dengan properti unik yang ada pada data
      return [...prevData, ...filteredNewData]; // Menggabungkan data lama dan yang baru tanpa duplikasi
    });
