    ...
    try {
      const response = await axios.get("http://116.206.212.234:4000/data-sektoral", {
        params: {
          ...
          page: currentPage,
          per_page: itemsPerPage, // Sesuaikan dengan parameter API yang benar
        },
      });
      ...
      if (totalItems) {
        setTotalData(parseInt(totalItems, 10)); // Set total data jika header tersedia
      } else {
        setTotalData(0); // Atur totalData ke 0 jika header tidak tersedia
      }
