...
const Sektoral = () => {
  ...
  useEffect(() => {
    const fetchDataOPD = async () => {
      try {
        ...
        // Hapus data OPD duplikat
        const uniqueOPDS = [...new Set(response.data.map((opd) => opd.id_opd))];
        setDataOPD(uniqueOPDS.map((id) => response.data.find((opd) => opd.id_opd === id)));
      } catch (error) {
        setError(error);
      } finally {
        setLoading(false);
  ...
  const handleOPDChange = async (e) => {
    ...
    if (opdId) {
      try {
        ...
        // Hapus data urusan duplikat
        const uniqueUrusans = [...new Set(response.data.map((urusan) => urusan.kode_urusan))];
        setDataUrusan(uniqueUrusans.map((kode) => response.data.find((urusan) => urusan.kode_urusan === kode)));
        setError(null);
      } catch (error) {
  ...
  const handleSearch = async (e = null) => {
    ...
    try {
      ...
      // Hapus duplikat data
      const uniqueDataHasil = [...new Set(response.data.map((item) => JSON.stringify(item)))].map((str) => JSON.parse(str));
      setDataHasil(uniqueDataHasil); // Simpan data unik
