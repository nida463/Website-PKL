...
const Sektoral = () => {
  ...
  const handleOPDChange = async (e) => {
    const selectedOPDValue = e.target.value;
    setSelectedOPD(selectedOPDValue);
    setSelectedUrusan('');  // Reset urusan ketika OPD berubah
    ...
    if (selectedOPDValue) {
      try {
        const response = await axios.get(`http://116.206.212.234:4000/data-sektoral/list-urusan-by-id-opd`, {
          params: { id_opd: selectedOPDValue }
        });
        setDataUrusan(response.data);  // Set data urusan berdasarkan OPD yang dipilih
      } catch (error) {
        setError(error);
      } finally {
        setLoading(false);  // Set status loading menjadi false setelah API selesai
      }
    } else {
      setDataUrusan([]);  // Kosongkan data urusan jika OPD tidak dipilih
        ...
        <form className="search-form" onSubmit={handleSearch}>
          <div className="search-inputs">
            ...
            <select 
              className="search-input" 
              value={selectedOPD} 
              onChange={handleOPDChange}  // Gunakan handleOPDChange di sini
              required
            >
